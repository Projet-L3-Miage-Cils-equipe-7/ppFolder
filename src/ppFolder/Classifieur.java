...
public class Classifieur {
	...
	private static void listFiles(File path) {
		...
		} else {listeOfPathsFiles.put(path.getAbsolutePath(), get_extension(path.toString()).toLowerCase());}
	}
	...
	public void trier() throws IOException {
		...
		if(!listeOfPathsFiles.isEmpty()) {
	        for (String regle : getRule().readSerializedRules()) {
	        	for (String path : listeOfPathsFiles.keySet()) {
